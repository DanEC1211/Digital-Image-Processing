...
while(1)
    ...
    Numerador = power(double(Imagen),2);
    Numerador= double(sum(Numerador));
    
    Denominador = (double(Imagen) - double(Imagen_Nueva));
    Denominador =  power(double(Denominador),2);
    Denominador = sum (Denominador);
  %  SumaImagen_Restada = sum(power((double(Imagen) - Imagen_Generada),2));
    
    SN = double(10 * log10( Numerador / Denominador));
